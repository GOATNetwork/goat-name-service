...
import {
    ...
    IERC165,
    IERC1155MetadataURI
} from "@ensdomains/ens-contracts/contracts/wrapper/ERC1155Fuse.sol";
...
import {BytesUtils_LEGACY} from "@ensdomains/ens-contracts/contracts/utils/BytesUtils_LEGACY.sol";
...
/// @title GoatNameWrapper
/// @notice Wraps `.goat` second-level names and subdomains into ENS-compatible ERC1155 fuse tokens.
/// @dev
/// - This is a fork of ENS `NameWrapper` with the `.eth`-specific 2LD path retargeted to `.goat`.
/// - Resolver compatibility is preserved through the upstream `INameWrapper` interface and fuse storage layout.
/// - Registration is not auto-wrapped in v1. Users receive the registrar ERC721 by default and may wrap later.
/// - The legacy `wrapETH2LD`/`unwrapETH2LD`/`registerAndWrapETH2LD` entrypoints are retained for interface compatibility and now operate on `.goat` 2LDs.
contract GoatNameWrapper is
    Ownable,
    ERC1155Fuse,
    INameWrapper,
    Controllable,
    ...
    ReverseClaimer
{
    ...
    string public constant name = "GoatNameWrapper";

    uint64 private constant GRACE_PERIOD = 90 days;
    bytes32 private constant GOAT_NODE =
        0x5b00879fb3cf6886271b2d0f62386525acf379cdbc8d0987bf35fd164141e507;
    bytes32 private constant GOAT_LABELHASH =
        0x2f00025a247bf7795d8888f7794dd86d493e249ae5f9154c404a64c67e3bc6c3;
    ...
    constructor(
        ...
        /* Burn PARENT_CANNOT_CONTROL and CANNOT_UNWRAP fuses for ROOT_NODE and GOAT_NODE and set expiry to max */
        ...
        names[ROOT_NODE] = "\x00";
        names[GOAT_NODE] = "\x04goat\x00";
    }

    /// @notice Returns the upstream ENS wrapper interface id published on `.goat`.
    /// @return id The `INameWrapper` ERC-165 interface id.
    function interfaceId() external pure returns (bytes4 id) {
        return type(INameWrapper).interfaceId;
    }
    ...
    function supportsInterface(
        ...
        return
            _interfaceId == type(INameWrapper).interfaceId ||
    ...
    /// @notice Gets the wrapped token owner for `id`.
    /// @param id The namehash token id tracked by the wrapper.
    /// @return owner The wrapped token owner.
    function ownerOf(
        uint256 id
    ...
    function canModifyName(
        ...
        return
            ...
            !_isGoat2LDInGracePeriod(fuses, expiry);
    }
    ...
    /// @notice Wraps a `.goat` second-level name through the legacy ENS interface name.
    /// @dev Can be called by the registrar owner or an authorised registrar operator. The function name is kept for `INameWrapper` compatibility.
    /// @param label The normalized `.goat` label to wrap.
    ...
    function wrapETH2LD(
        ...
        if (
            ...
            revert Unauthorised(
                _makeNode(GOAT_NODE, bytes32(tokenId)),
                msg.sender
            );
        }
    ...
    /// @dev Registers a new `.goat` second-level domain and wraps it.
    ///      Only callable by authorised controllers.
    /// @param label The label to register (for example, `foo` for `foo.goat`).
    /// @param wrappedOwner The owner of the wrapped name.
    /// @param duration The duration, in seconds, to register the name for.
    /// @param resolver The resolver address to set on the ENS registry (optional).
    /// @param ownerControlledFuses Initial owner-controlled fuses to set
    /// @return registrarExpiry The expiry date of the new name on the `.goat` registrar, in seconds since the Unix epoch.
    function registerAndWrapETH2LD(
        ...
        address resolver,
        uint16 ownerControlledFuses
    ) public onlyController returns (uint256 registrarExpiry) {
        uint256 tokenId = uint256(keccak256(bytes(label)));
    ...
    /// @notice Renews a `.goat` second-level domain.
    /// @dev Only callable by authorised controllers.
    /// @param tokenId The hash of the label to renew (for example, `keccak256("foo")` for `foo.goat`).
    /// @param duration The number of seconds to renew the name for.
    /// @return expires The expiry date of the name on the `.goat` registrar, in seconds since the Unix epoch.
    function renew(
        uint256 tokenId,
        uint256 duration
    ...
    /// @notice Wraps a non-`.goat` domain, such as a DNSSEC-imported name or a subdomain.
    ...
    /// @notice Unwraps a wrapped `.goat` second-level name through the legacy ENS interface name.
    /// @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
    /// @param labelhash Labelhash of the `.goat` domain
    /// @param registrant Sets the owner in the `.goat` registrar to this address
    /// @param controller Sets the owner in the registry to this address
    function unwrapETH2LD(
        bytes32 labelhash,
        address registrant,
        address controller
    ) public onlyTokenOwner(_makeNode(GOAT_NODE, labelhash)) {
    ...
    /// @notice Unwraps a non-`.goat` domain, such as a DNSSEC-imported name or a subdomain.
    /// @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
    ...
    /// @notice Upgrades any wrapped domain, including `.goat` names, DNSSEC names, and subdomains.
    ...
    function onERC721Received(
        ...
        // Only `.goat` registrar ERC721 tokens can be wrapped through this hook.
        ...
        _wrapGoat2LD(label, owner, ownerControlledFuses, expiry, resolver);
    ...
    function _beforeTransfer(
        ...
        // For this check, treat `.goat` 2LDs as expiring at the start of the grace period.
        if (fuses & IS_DOT_ETH == IS_DOT_ETH) {
            expiry -= GRACE_PERIOD;
        }
    ...
    function _checkParentFuses(
        ...
        bool isBurningParentControlledFuses =
    ...
    function _normaliseExpiry(
        ...
        // `.goat` names check the registrar, while non-`.goat` names are capped by their parent expiry.
        if (expiry > maxExpiry) {
            expiry = maxExpiry;
        }
    ...
    function _wrapGoat2LD(
        ...
        bytes32 labelhash = keccak256(bytes(label));
        bytes32 node = _makeNode(GOAT_NODE, labelhash);
        // Hardcode the DNS-encoded `.goat` suffix for gas savings.
        bytes memory _name = _addLabel(label, "\x04goat\x00");
        names[node] = _name;
    ...
    function _isGoat2LDInGracePeriod(
        uint32 fuses,
        uint64 expiry
    ...
    /// @notice Wraps a `.goat` second-level name using a `.goat`-named helper alias.
    /// @dev Calls the legacy `wrapETH2LD` entrypoint to preserve `INameWrapper` compatibility.
    /// @param label The normalized `.goat` label to wrap.
    /// @param wrappedOwner The wrapped token recipient.
    /// @param ownerControlledFuses The owner-controlled fuses to burn on wrap.
    /// @param resolver The resolver to install for the wrapped name.
    /// @return expiry The wrapped name expiry, including the registrar grace period.
    function wrapGoat2LD(
        string calldata label,
        address wrappedOwner,
        uint16 ownerControlledFuses,
        address resolver
    ) external returns (uint64 expiry) {
        return wrapETH2LD(label, wrappedOwner, ownerControlledFuses, resolver);
    }

    /// @notice Registers and wraps a `.goat` second-level name using a `.goat`-named helper alias.
    /// @dev Calls the legacy `registerAndWrapETH2LD` entrypoint to preserve `INameWrapper` compatibility.
    /// @param label The normalized `.goat` label to register.
    /// @param wrappedOwner The wrapped token recipient.
    /// @param duration The registration duration in seconds.
    /// @param resolver The resolver to install for the wrapped name.
    /// @param ownerControlledFuses The owner-controlled fuses to burn on wrap.
    /// @return registrarExpiry The registrar expiry timestamp before wrapper grace-period extension.
    function registerAndWrapGoat2LD(
        string calldata label,
        address wrappedOwner,
        uint256 duration,
        address resolver,
    ...
    /// @notice Unwraps a `.goat` second-level name using a `.goat`-named helper alias.
    /// @dev Calls the legacy `unwrapETH2LD` entrypoint to preserve `INameWrapper` compatibility.
    /// @param labelhash The keccak256 hash of the normalized `.goat` label.
    /// @param registrant The recipient of the registrar ERC721.
    /// @param controller The recipient of the ENS registry ownership.
    function unwrapGoat2LD(
        bytes32 labelhash,
        address registrant,
        address controller
    ) external {
        unwrapETH2LD(labelhash, registrant, controller);
    }
}
